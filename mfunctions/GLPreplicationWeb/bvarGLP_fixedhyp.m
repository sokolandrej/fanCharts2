function r = bvarGLP_fixedhyp(y,lags,varargin)
...
MIN.lambda = lambda0;
MIN.alpha = alpha0; %note that alpha is hard-coded in the logMLVAR function, change if needed!
MIN.theta =  theta0;
MIN.miu = miu0;
MAX.lambda=lambda0;
MAX.miu=miu0;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [fh,xh,gh,H,itct,fcount,retcodeh] = csminwel('logMLVAR_formin',x0,H0,[],1e-16,1,y,x,lags,T,n,b,MIN,MAX,SS,Vc,pos,mn,sur,noc,y0,hyperpriors,priorcoef);
...
r.postmax.miu=miu0;
r.postmax.alpha=alpha0; %note that alpha is hard-coded in the logMLVAR function, change if needed!
...
%         [logMLold,betadrawold,sigmadrawold]=logMLVAR_formcmc(postmode,y,x,lags,T,n,b,MIN,MAX,SS,Vc,pos,mn,sur,noc,y0,...
...
%         P(i,:)=mvnrnd(P(i-1,:),HH*const^2,1);
        [~,betadrawnew,sigmadrawnew]=logMLVAR_formcmc(postmode,y,x,lags,T,n,b,MIN,MAX,SS,Vc,pos,mn,sur,noc,y0,...
            max([MCMCfcast,MCMCstorecoeff]),hyperpriors,priorcoef);
